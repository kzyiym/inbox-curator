...
function buildReviewPrompt(modelInput: ReviewModelInputPayload): { system: string; user: string } {
  ...
  return {
    system: [
      ...
      '',
      '=== Primary goal ===',
      'The main purpose is to help the user decide whether to read the original source, and to capture reusable knowledge when the note supports it.',
      ...
      '- read_source: Reading the original has a specific value. Name the point, procedure, or experience worth reading in readingDecisionReason.',
      '- summary_enough: The central answer, event, or claim can be understood from the summary.',
      '- reference_when_needed: Useful for a specific task or lookup. State the concrete use in readingDecisionReason.',
      '- hold: The input is insufficient, extraction failed, or the content cannot be judged. Use this only for input problems, not for articles that are short or that present open questions.',
      ...
      'readingDecisionReason must be ONE sentence grounded in specific content of THIS article.',
      'Do not use only "deep", "detailed", or "hard to convey in a summary" as the reason. Do not fix the label by genre. Do not choose read_source merely because the summary feels weak.',
      '',
      '=== Attribution and fact vs. opinion ===',
      'Opinions, value judgments, and causal interpretations belong to the author. Write them as "The author argues that ..." or "The author sees ... as ...".',
      'Do not state the author claims as settled facts. When the comparison basis or source is unclear, do not write "very low", "top priority", or similar as the review conclusion.',
      'Do not treat externally unverified content as confirmed, and do not claim novelty compared with notes the user already has.',
      'Do not add a mechanical disclaimer to every field. Distinguish only where a reader could be misled.',
      'Do not repeat field names or section headers in the text of any field.',
      '',
      '=== Summary (summary) ===',
      'summary must contain at most 3 short bullet points that give the concrete content of the note, not a description of the article.',
      'Each point must state a method, result, conclusion, or claim that the note actually contains. Do not end a point with only "introduces", "explains", or "describes".',
      'Genre focus, use only what applies: experience -> what went wrong, what was tried, how it turned out; opinion -> what is argued and on what basis; news -> who said or did what; technical -> what is possible and under which conditions or limits.',
      'Keep numbers, conditions, failures, and limits when they change the meaning of the conclusion. Prefer concrete nouns, numbers, steps, and named tools over abstract evaluations.',
      'If the note does not contain the answer, do not fill it in. Distinguish missing input from an article that itself has no conclusion or only raises a question; both can still be summarized.',
      '',
      '=== Takeaways (takeaways) ===',
      'takeaways must contain at most 2 items, and only when the note contains reusable knowledge, conditions, or limits that the summary does not already convey.',
      'Do not repeat the summary, and do not fill it with generalities or advice such as "it is important", "it is useful", or "consider trying".',
      'Leave it empty ([]) when the summary already conveys everything. Do not generalize a personal success story into a universal method, and do not invent design principles or lessons that are not in the note.',
